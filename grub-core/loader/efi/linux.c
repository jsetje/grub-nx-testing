				 ...
				 int *nx_supported)
{
  ...
  if ((doshdr->magic & 0xffff) != GRUB_PE32_MAGIC)
				      ...
				      int nx_supported)
{
  ...
  grub_dprintf ("nx", "Setting attributes for 0x%"PRIxGRUB_ADDR"-0x%"PRIxGRUB_ADDR" to r%cx\n",
		    (grub_addr_t)kernel_addr, (grub_addr_t)kernel_addr + kernel_size - 1,
		    (kernel_set_attrs & GRUB_MEM_ATTR_W) ? 'w' : '-');
  grub_update_mem_attrs ((grub_addr_t)kernel_addr, kernel_size,
			 kernel_set_attrs, kernel_clear_attrs);

  grub_get_mem_attrs ((grub_addr_t)kernel_addr, 4096, &attrs);
		...
		int argc, char *argv[])
{
  ...
  if (grub_arch_efi_linux_load_image_header (file, &lh, nx_supported) != GRUB_ERR_NONE)
#if !defined(__i386__) && !defined(__x86_64__)
    goto fail;
#else
    goto fallback;
